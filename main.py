...
def voice():
    ...
    # Tell Twilio to connect the call to our WebSocket server
    connect = Connect()
    connect.stream(url="wss://ded/media")  # <- change to ngrok URL
    response.append(connect)
                                                        
    print("Generated TwiML:", str(response))
...
async def websocket_server():
    """Runs the Twilio <-> Deepgram WebSocket bridge."""
...
if __name__ == "__main__":
    ...
    loop.create_task(websocket_server())   # Start websocket
    app.run(host="0.0.0.0", port=8080)     # Start Flask webhook
